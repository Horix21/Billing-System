...
def submit():
    ...
    if len(entryOptimus.get()) != 0:
        e=int(entryOptimus.get())
        sum = sum + e*1000000
    
    if len(entryWatjus.get()) != 0:
        e=int(entryWatjus.get())
        sum = sum + e*30

    if len(entryColdDog.get()) != 0:
        e=int(entryColdDog.get())
        sum = sum + e*69.42

    if len(entryDog.get()) != 0:
        e=int(entryDog.get())
        sum = sum + e*55
...
var7=IntVar()
ColdCoffe = Checkbutton(root, text = 'Cold Coffee',variable=var7)
...
Optimus = Checkbutton(root, text = 'Optimus Prime')
Optimus.grid(row = 0, column=6,sticky=W)
entryOptimus = Entry(root, bd=3)
entryOptimus.grid(row = 0, column=7)

WatermelJuice = Checkbutton(root, text = 'Watermelon Juice')
WatermelJuice.grid(row = 1, column=6,sticky=W)
entryWatjus = Entry(root, bd=3)
entryWatjus.grid(row = 1, column=7)

ColdDog = Checkbutton(root, text = 'Cold Dog')
ColdDog.grid(row = 2, column=6,sticky=W)
entryColdDog = Entry(root, bd=3)
entryColdDog.grid(row = 2, column=7)

upDog = Checkbutton(root, text = 'Up Dog')
upDog.grid(row = 3, column=6,sticky=W)
entryDog = Entry(root, bd=3)
entryDog.grid(row = 3, column=7)

#the button

submit = Button(root, text="submit", padx=450, pady=10, command=submit)
submit.grid(row=8, column=0, columnspan=8)
